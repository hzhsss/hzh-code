...
随着H5的发展 ，允许 javaScript 创建多个线程。于是 ， JS 中出现了 **同步** 与 **异步**
...
主线程会不断的重复获得任务、执行任务，这种机制被称为 **事件循环（event loop）**
...
| 对象方法           | 返回值                                       |
| ------------------ | -------------------------------------------- |
| location.assign()  | 跟 href 一样，可以正常跳转页面（重定向页面） |
